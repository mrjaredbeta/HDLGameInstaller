...
void ConvertMcTitle(const mcIcon *icon, wchar_t *title1, wchar_t *title2)
{
    ...
    if (icon->nlOffset > 0 && icon->nlOffset < sizeof(icon->title)) { // The title is split with a newline.
        ...
        if (IsASCIITitle(title1Raw) && (title2Raw == NULL || IsASCIITitle(title2Raw))) { // Convert the title to ASCII, if the title can be represented in ASCII.
        ...
        } else { // Otherwise, just copy the title.
    ...
    } else { // Title has no newline.
        len            = SJISToUnicode((const char *)icon->title, sizeof(icon->title), SaveDataTitle, sizeof(SaveDataTitle) / sizeof(wchar_t) - 1);
        title1Raw[len] = '\0';

        if (IsASCIITitle(SaveDataTitle)) { // Convert the title to ASCII, if the title can be represented in ASCII.
        ...
        } else { // Otherwise, just split the title into 2.
...
static int GenerateHDDIconSysFileFromMCSave(const mcIcon *McIconSys, char *HDDIconSys, const wchar_t *OSDTitleLine1, const wchar_t *OSDTitleLine2)
{
    ...
    IconSysData.bgcol0[0]      = McIconSys->bgCol[0][0];
    IconSysData.bgcol0[1]      = McIconSys->bgCol[0][1];
    IconSysData.bgcol0[2]      = McIconSys->bgCol[0][2];
    IconSysData.bgcol1[0]      = McIconSys->bgCol[1][0];
    IconSysData.bgcol1[1]      = McIconSys->bgCol[1][1];
    IconSysData.bgcol1[2]      = McIconSys->bgCol[1][2];
    IconSysData.bgcol2[0]      = McIconSys->bgCol[2][0];
    IconSysData.bgcol2[1]      = McIconSys->bgCol[2][1];
    IconSysData.bgcol2[2]      = McIconSys->bgCol[2][2];
    IconSysData.bgcol3[0]      = McIconSys->bgCol[3][0];
    IconSysData.bgcol3[1]      = McIconSys->bgCol[3][1];
    IconSysData.bgcol3[2]      = McIconSys->bgCol[3][2];
...
int LoadMcSaveSys(char *SaveFolderPath, mcIcon *McSaveIconSys, const char *DiscID)
{
    ...
    for (i = 0, result = -ENOENT; i < 2; i++) {
        if ((result = FindGameSaveFolder(i, SaveFolderPath, 38, DiscID)) >= 0) {
            if ((result = LoadMcSaveSysFromPath(SaveFolderPath, McSaveIconSys)) == 0)
                break;
        } else
            DEBUG_PRINTF("Memory card save folder not found: %d\n", result);
    }
...
static int GenerateIconFilesFromMCSave(const char *SaveFolderPath, const mcIcon *McSaveIconSys, struct OSDResourceFileEntry *InstFileList, const wchar_t *OSDTitleLine1, const wchar_t *OSDTitleLine2)
{
    ...
    if ((IconSysFD = fileXioOpen(McPath, O_RDONLY)) >= 0) {
        ...
        if (strcmp(McSaveIconSys->del, McSaveIconSys->view) != 0) {
            ...
            DEBUG_PRINTF("No delete icon found.\n");
    } else
        DEBUG_PRINTF("Unable to load list view icon: %d\n", IconSysFD);
...
int LoadOSDResource(unsigned int index, struct OSDResourceFileEntry *InstFileList, void *buffer, unsigned int length, unsigned char IsAllocatedBuffer)
{
    ...
    if (index < NUM_OSD_FILES_ENTS) {
        if (InstFileList[index].buffer != NULL && (InstFileList[index].flags & FILE_FLAGS_ALLOCATED))
            free(InstFileList[index].buffer);
...
int InstallGameInstallationOSDResources(const char *partition, const struct GameSettings *GameSettings, const mcIcon *McSaveIconSys, const char *SaveDataPath)
{
    ...
    if (McSaveIconSys == NULL) {
        ...
        if (PrepareOSDResources(SaveDataPath, McSaveIconSys, GameSettings->OSDTitleLine1, GameSettings->OSDTitleLine2, InstFileList) == ICON_SOURCE_SAVE) {
            ...
            result = ICON_SOURCE_DEFAULT; // Leave the caller to decide on what to do next.
    }
